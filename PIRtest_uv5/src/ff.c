...
/*-----------------------------------------------------------------------*/
//这个只能得到多少簇，其中FAT32是一个簇8个扇区，FAT16貌似一个簇64个扇区
FRESULT f_getfree (
...
//这个我是自己添加的，得到簇数和 每簇多少扇区数，为了方便得到最终多少个free字节
//我只管FAT16 和FAT32的里，这个程序主要从上面f_getfree拷贝过来，这不过增加了一个secperclu,
//按理来说还应该增加一个bytepersec，但是因为一般都是512字节，还有就是因为这里的SD卡字节和这个ff.c（这两个都是由stm公司提供的）不配套，所以也只能适用于512的情况
FRESULT f_getfreeCluandSec (
	const TCHAR *path,	/* Pointer to the logical drive number (root dir) */
	DWORD *nclst,		/* Pointer to the variable to return number of free clusters */
	DWORD *secperclu,		/*  */
	FATFS **fatfs		/* Pointer to pointer to corresponding file system object to return */
