    ...
    for(uint j=0;j<array_size;j++){
        ...
        if(data_tempa[j]<0){
            if (data_tempa[j]<=-10) {
                ...
                packet_pointer++;
                packet_tosend[packet_pointer] = '9';    // if less than -10, put -99 there.
                packet_pointer++;
            ...
            {
                packet_tosend[packet_pointer] = value_char[0];  // -8.1
                packet_pointer++;
...
void setup()
{
    ...
    PWR2745.initPower(PWR_BAT|PWR_RTC);//±ÿ–Î’‚¡Ω∏ˆ∂ºø™∂¡µÁ¡˜ ˝æ›≤≈ƒ‹∂‘£¨’‚∏ˆ «“ÚŒ™RTC∫Õ∂¡µÁ¡˜µÁ—π–æ∆¨π´”√I2C,“™ «÷ªø™“ª∏ˆ∂¡ ˝æ›æÕª·”–Œ Ã‚
