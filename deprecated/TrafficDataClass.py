...
class TrafficData:
    ...
    def load_npy_data(self, file_name_str=None, name=None):
        """
        ...
        :param name: the name of the data saved in self.PIR
        :return:
        """
        if file_name_str is not None and exists(file_name_str):
            data = np.load(file_name_str)

            self.PIR[name] = {'time':[], 'Ta':[], 'raw_data':[], 'cleaned_data':[]}
            # save into structure
            for entry in data:
                self.PIR[name]['time'].append(entry[0])
                self.PIR[name]['raw_data'].append(entry[1])
                self.PIR[name]['Ta'].append(entry[2])

            # change to array
            self.PIR[name]['time'] = np.array(self.PIR[name]['time'])
            self.PIR[name]['raw_data'] = np.array(self.PIR[name]['raw_data'])
            self.PIR[name]['Ta'] = np.array(self.PIR[name]['Ta'])
